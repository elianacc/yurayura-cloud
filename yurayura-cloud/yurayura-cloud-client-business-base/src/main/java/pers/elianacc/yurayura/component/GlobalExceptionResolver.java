    ...
    public ModelAndView resolveException(HttpServletRequest request, HttpServletResponse response, Object handler,
        ...
        if (exception instanceof RepeatSubmitException) {
            ...
            if (businessException.getErrorCode() == 500) {
                response.getWriter().write(JSON.toJSONString(ApiResult.fail(businessException.getErrorMsg())));
            } else {
                response.getWriter().write(JSON.toJSONString(ApiResult.warn(businessException.getErrorMsg())));
            }
            return new ModelAndView();
        } else {
            // 异常为系统异常
